...
public partial class PathBuilder
{
	public static void DrawTheArc()
	{
		...
		var spath         = new Path(SegmentType.Cut, MakePoint(arc.GetPointFromAngle(arc.StartAngle), 3)); // TODO: figure out the right segment type
	...
	private static IEnumerable<Path> GetPaths(DxfFile dxf)
	{
	...
	private static IEnumerable<Path> GetPaths(DxfEntity entity)
	{
		switch (entity) {
			case DxfArc arc:
	...
	private static Path ExpandArc2(DxfArc arc)
	{
	...
	private static Path ExpandArc(DxfArc arc)
	{
		// for an arc, 0 degrees is in the +X direction, increasing in a CCW rotation
		var circumference = 2 * Math.PI * arc.Radius;
		var sweep         = CalculateSweep(arc);
